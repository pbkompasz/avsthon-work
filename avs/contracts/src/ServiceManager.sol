...
contract ServiceManager is ServiceManagerBase {
    ...
    function transferEscrow(string contractId) external payable returns (bool) {
        uint8[] memory confidentialContract = ().getContract(contractId);
        Contract memory parsedContract = ().parseJsonLdContract(
            confidentialContract
        );
        require(
            msg.value == parsedContract.totalCost,
            "Escrow payment not matching"
        );
    }
}
